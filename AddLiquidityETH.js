...
const factory = new Contract(FACTORY_ADDRESS, factoryArtifact.abi, owner)
 ...
 const tokenContractAddress = '0xEBE8B46A42F05072b723b00013ff822B2af1B5cb';
 const amountTokenDesired = ethers.parseUnits("10", 18);
 const amountTokenMin = ethers.parseUnits("10", 18);
 const amountETHMin = ethers.parseUnits("1", 17);
 const to  = '0xaB9f3d7402c43AA472EEE03b176A97DFc920AECd';
 const deadline = Math.floor(Date.now() / 1000) + 10 * 60;

  //approving tx to provide liquidity 
  const MaxUint256 =   "0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff";
  const approvalTx1 = await bger.approve(ROUTER_ADDRESS, MaxUint256, {gasLimit: 300000});
 const stats = await approvalTx1.wait();
 console.log(stats)

  //adding new liquidity if not generate new pair 
  //address token, uint amountTokenDesired,uint amountTokenMin, uint amountETHMin, address to,uint deadline
  const addLiquidityTx = await router.connect(owner).addLiquidityETH(tokenContractAddress, amountTokenDesired, amountTokenMin, amountETHMin, to, deadline); 
